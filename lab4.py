...
#        train_file.writerow([b0.x, b0.y, b0.vx, b0.vy, dir, p0.y, Ball.RADIUS, Paddle.L, Paddle.STEP, CONSTS.WIDTH, CONSTS.HEIGHT, CONSTS.BORDER, CONSTS.VELOCITY, CONSTS.FPS])
    ...
    if not train:
        ...
        model = load('C:/Users/Robert Martini/Desktop/lab4/mymodel.joblib')

        #data = pd.read_csv("C:/Users/Robert Martini/PycharmProjects/TrainModel/pong_data.csv")
    ...
    while running:
        ...
        if not train: #deploy model
            ...
            #full write:
            # train_file.writerow([b0.x, b0.y, b0.vx, b0.vy, dir, p0.y, Ball.RADIUS, Paddle.L, Paddle.STEP, CONSTS.WIDTH, CONSTS.HEIGHT, CONSTS.BORDER, CONSTS.VELOCITY, CONSTS.FPS])
            X = {'ball_x': b0.x, 'ball_y': b0.y, 'ball_vx': b0.vx, 'ball_vy': b0.vy, 'dir': dir, 'Ball.RADIUS': Ball.RADIUS, 'Paddle.L': Paddle.L, 'Paddle.STEP': Paddle.STEP, 'WIDTH': CONSTS.WIDTH, 'HEIGHT': CONSTS.HEIGHT, 'BORDER': CONSTS.BORDER, 'VELOCITY ': CONSTS.VELOCITY, 'FPS': CONSTS.FPS}
            X = pd.DataFrame([X])
            y = model.predict(X.values)[0]

            #Determines whether paddle dir should be up or down based on prev and current y_paddle prediction
            #Added buffer to keep paddle from flickering
            if abs(p0.y - y) < 7:
              pass
            elif p0.y < y:
                dir = 1
